...
/** Every realm's ring buffer, as the setup page reads them: `gjLog:<realm>` → lines. */
const logs = (p: Peer) => p.extPage.evaluate(() =>
  chrome.storage.session.get(null).then((all: Record<string, unknown>) => Object.fromEntries(Object.entries(all).filter(([k]) => k.startsWith('gjLog:')))),
...
test('diagnostics: every realm reaches session storage, pages forward through the offscreen document, the setup page assembles one report', async () => {
  ...
  try {
    ...
    await waitForCondition(async () => ((await logs(b))['gjLog:offscreen'] ?? []).some((l) => /\[sync@\w{4}\] local seek at \d+ms$/.test(l)), { label: 'page line forwarded into the offscreen buffer' });
    ...
    expect((la['gjLog:background'] ?? []).some((x) => x.includes('[background] navigation ')), 'background buffer has the navigation').toBe(true);
