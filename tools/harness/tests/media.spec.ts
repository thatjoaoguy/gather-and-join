...
async function waitForAudioFrom(peers: Array<{ gaj: any; peerId: string; index: number }>, timeout = 20_000) {
  for (const p of peers) {
    for (const q of peers) {
      if (p === q) continue;
      await waitForCondition(async () => {
        const a = await p.gaj('getRemoteAudio') as Audio;
...
test('mesh audio: each peer receives the right peer\'s tone; ice/signaling stable', async () => {
  ...
  try {
    ...
    for (const p of party.peers) {
      const st = await p.gaj('getPeerStats') as Stats;
...
test('camera: opt-in video arrives with the right colour and framesDecoded increases', async () => {
  ...
  try {
    ...
    await waitForMesh(party.peers);
    await a.gaj('setCamera', true);
    await waitForCondition(async () => { const v = await b.gaj('getRemoteVideo') as Video; return colorMatches(v[a.peerId]?.rgb ?? null, PEER_COLORS[0]!); }, { timeout: 15_000, label: 'peer2 sees peer1 colour' });
    const f1 = ((await b.gaj('getRemoteVideo')) as Video)[a.peerId]!.framesDecoded;
    await sleepMs(1000);
    const f2 = ((await b.gaj('getRemoteVideo')) as Video)[a.peerId]!.framesDecoded;
    expect(f2).toBeGreaterThan(f1);
    // The page-side tile (loopback) renders too — on the receiver, and as a mirrored "You" self-view on the sender.
    await waitForCondition(() => b.page.evaluate(() => { const host = document.getElementById('gaj-tiles'); const v = host?.shadowRoot?.querySelector<HTMLVideoElement>('.tile.has-video:not(.self) video'); return !!v && v.videoWidth > 0; }), { timeout: 10_000, label: 'tile rendered in page sidebar' });
    await waitForCondition(() => a.page.evaluate(() => { const host = document.getElementById('gaj-tiles'); const t = host?.shadowRoot?.querySelector('.tile.self.has-video'); const v = t?.querySelector('video'); return !!v && v.videoWidth > 0 && t?.querySelector('.name')?.textContent === 'peer1'; }), { timeout: 10_000, label: 'self-view tile on sender' });
    // The receiver's own tile is a name placeholder (its camera is off); the sidebar pushed the page over.
    expect(await b.page.evaluate(() => !document.getElementById('gaj-tiles')?.shadowRoot?.querySelector('.tile.self')?.classList.contains('has-video'))).toBe(true);
    ...
    await a.gaj('setCamera', false);
    await waitForCondition(() => b.page.evaluate(() => { const host = document.getElementById('gaj-tiles'); return !host?.shadowRoot?.querySelector('.tile.has-video'); }), { timeout: 10_000, label: 'video removed on receiver' });
    await waitForCondition(() => a.page.evaluate(() => { const host = document.getElementById('gaj-tiles'); return !host?.shadowRoot?.querySelector('.tile.has-video'); }), { timeout: 10_000, label: 'self-view video removed' });
    await waitForMesh(party.peers);
...
test('self-view alone: a lone peer with the camera on sees its own tile', async () => {
  ...
  try {
    ...
    await a.gaj('setCamera', true);
    await waitForCondition(() => a.page.evaluate(() => { const t = document.getElementById('gaj-tiles')?.shadowRoot?.querySelector('.tile.self.has-video'); const v = t?.querySelector('video'); return !!v && v.videoWidth > 0; }), { timeout: 10_000, label: 'self-view without any peers' });
    await a.gaj('setCamera', false);
    await waitForCondition(() => a.page.evaluate(() => !document.getElementById('gaj-tiles')?.shadowRoot?.querySelector('.tile.has-video')), { timeout: 10_000, label: 'self-view video removed' });
    // Leaving the room removes the sidebar and restores the page width.
    await a.gaj('leaveRoom');
    await waitForCondition(() => a.page.evaluate(() => !document.getElementById('gaj-tiles') && Math.round(document.documentElement.getBoundingClientRect().width) === innerWidth), { timeout: 10_000, label: 'sidebar removed, page restored' });
...
test('fullscreen: the sidebar re-parents into the fullscreen element and comes back out', async () => {
  ...
  try {
    ...
    await waitForMesh(party.peers);
    await a.gaj('setCamera', true);
    await waitForCondition(() => b.page.evaluate(() => !!document.getElementById('gaj-tiles')?.shadowRoot?.querySelector('.tile.has-video')), { timeout: 10_000, label: 'peer2 sees a video tile' });
    ...
    await waitForCondition(() => b.page.evaluate(() => {
      const host = document.getElementById('gaj-tiles');
    ...
    expect(await b.page.evaluate(() => { const v = document.getElementById('gaj-tiles')!.shadowRoot!.querySelector<HTMLVideoElement>('.tile.has-video video')!; const r = v.getBoundingClientRect(); const el = document.elementFromPoint(r.left + r.width / 2, r.top + r.height / 2); return el?.id === 'gaj-tiles'; })).toBe(true);
    // The fullscreen element's other children are narrowed so the sidebar covers nothing.
    const widths = await b.page.evaluate(() => ({ inner: innerWidth, kids: Array.from(document.fullscreenElement!.children).map((c) => `${c.tagName}#${c.id} w=${Math.round(c.getBoundingClientRect().width)} shrunk=${(c as HTMLElement).dataset.gajShrunk ?? '-'}`) }));
    ...
    const visibleKids = widths.kids.filter((k) => !k.includes('gaj-tiles') && !k.includes('w=0 '));
    ...
    await waitForCondition(() => b.page.evaluate(() => { const host = document.getElementById('gaj-tiles'); return !document.fullscreenElement && host?.parentElement === document.body && !host.classList.contains('overlay') && Math.round(document.documentElement.getBoundingClientRect().width) === innerWidth - 240 && !document.querySelector('#player [data-gaj-shrunk]') && document.querySelector('video')!.style.width === '100%' && Math.round(document.querySelector('video')!.getBoundingClientRect().width) === Math.round(document.getElementById('player')!.getBoundingClientRect().width); }), { timeout: 5000, label: 'sidebar back in body, page shrunk again, video width restored exactly' });
...
test('perfect negotiation: 20 simultaneous camera toggles with jitter end connected and stable', async () => {
  ...
  try {
    ...
    const toggler = async (p: typeof a) => { for (let i = 0; i < 20; i++) { await p.gaj('setCamera', i % 2 === 0); await sleepMs(jitter()); } };
...
test('ducking: tone onset ducks the video within 300ms; offset restores within 1.5s', async () => {
  ...
  try {
    ...
    await waitForCondition(async () => (await a.gaj('getVolume') as number) > 0.95, { timeout: 10_000, label: 'silence baseline' });
    await waitForCondition(async () => (await a.gaj('getDiag') as any).localMicLevel < 0.005, { timeout: 10_000, label: 'mic silent' });
    // Onset: poll fast; from the moment the mic level rises, volume must be < 0.35 within 300ms.
    let onsetAt = 0;
    await waitForCondition(async () => { const d = await a.gaj('getDiag') as any; if (d.localMicLevel > 0.02) { onsetAt = Date.now(); return true; } return false; }, { timeout: 10_000, interval: 20, label: 'tone onset' });
    await waitForCondition(async () => (await a.gaj('getVolume') as number) < 0.35, { timeout: 300 + 100, interval: 10, label: 'ducked within 300ms' });
    ...
    await waitForCondition(async () => (await a.gaj('getDiag') as any).localMicLevel < 0.005, { timeout: 5000, interval: 20, label: 'tone offset' });
    const offsetAt = Date.now();
    await waitForCondition(async () => (await a.gaj('getVolume') as number) > 0.95, { timeout: 1500 + 100, interval: 10, label: 'restored within 1.5s' });
