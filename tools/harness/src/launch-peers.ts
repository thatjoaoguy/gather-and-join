 ...
 *   pnpm --filter @gaj/harness launch 3      # then this
 *   REAL_MEDIA=1 pnpm --filter @gaj/harness launch 2   # real mic/camera instead of fixtures
 *
 * Peer 1 creates a room (ROOM_CODE to pin one); the others join it. Ctrl-C closes everything.
 */
import { generateRoomCode } from '@gaj/shared';
...
const peers: Peer[] = [];
for (let i = 0; i < n; i++) {
  const p = await launchPeer(i, { headless: false, sabotage: (process.env.GAJ_SABOTAGE as never) ?? null, realMedia: process.env.REAL_MEDIA === '1' });
  peers.push(p);
  await openPlayer(p);
  if (i === 0) await p.gaj('createRoom', code, p.name);
  else await p.gaj('joinRoom', code, p.name);
  await waitForCondition(async () => ((await p.gaj('getSnapshot')) as any)?.room?.code === code, { timeout: 30_000, label: `${p.name} in room` });
